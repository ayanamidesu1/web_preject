            ...
            if is_authenticated:
                with connection.cursor() as cursor:
                    ...
                    if account_permissions in ['1', '2', 1, 2]:
                        ...
                        # 插入模糊搜索条件
                        filters.append(
                            "(illustration_work.Illustration_id LIKE %s OR illustration_work.name LIKE %s OR "
                            "users.username LIKE %s OR users.userid LIKE %s)")
                        params.extend([search_key, search_key, search_key, search_key])

                        # 插入状态过滤条件
                        if work_status != 'all':
                            filters.append("illustration_work.work_approved = %s")
                            params.append(work_status)

                        # 拼接查询语句
                        sql_query = f'''
                            SELECT illustration_work.*, users.userid, users.user_avatar, users.username
