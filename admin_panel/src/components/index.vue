...
const getWsConfig = () => ({
  ...
  target_url: `wss://www.sunyuanling.com/ws/chat?role=sys_admin&user_id=${
...
const handleReconnect = () => {
  ...
  const delay = Math.min(
    baseRetryDelay * Math.pow(2, retryCount.value - 1),
    30000
  ); // 指数退避，最大30秒

  console.log(
    `将在 ${Math.round(delay / 1000)} 秒后尝试第 ${retryCount.value} 次重连...`
  );
