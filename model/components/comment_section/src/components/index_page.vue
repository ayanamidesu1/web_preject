...
import { get_comment, add_comment } from './js/get_comment';
...
onMounted(async () => {
  comment_list.value = await get_comment(props.work_id, props.work_type, 'sunyuanling')
...
async function get_more_comment() {
  limit.value += 3;
  //offset.value+=3;
  comment_list.value = await get_comment(props.work_id, props.work_type, 'sunyuanling', limit.value, offset.value)
}
...
async function get_sub_msg_ready_ok_status(status) {
  ...
  console.log(sub_reply_comment.value)
...
<template>
  <div class="index">
    ...
    <div class="content">
      <send_msg_box @send_msg="receive_msg" :user_avatar_path="'work_like.svg'" :msg_info="msg_info"
