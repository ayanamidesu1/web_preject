<template>
    <div class="order-details-container">
        ...
        <div class="order-content">
            ...
            <div class="section">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item">
                        ...
                        <span class="value status-badge" :class="getStatusClass(order_info.status)">
                            {{ getStatusText(order_info.status) }}
                        </span>
                    </div>
                    <div class="info-item">
                        <span class="label">金额:</span>
                        <span class="value amount">¥{{ formatAmount(order_info.amount_of_money) }}</span>
                    </div>
                    <div class="info-item">
            ...
            <div class="section" v-if="order_info.status==4||order_info.status==true">
            ...
            <div class="action-buttons">
                ...
                <button v-if="canConfirm" @click="confirmCompletion" class="btn confirm-btn">
                    确认完成
                </button>
                <button v-if="canReject" @click="rejectCompletion" class="btn reject-btn">
                    拒绝完成
                </button>
...
const canCancel = computed(() => {
    return [3].includes(order_info.value.status);
});

// 是否可以确认完成
const canConfirm = computed(() => {
    return (
        order_info.value.status === 4 && store.user.userid === order_info.value.user_id
    );
});
...
const confirmCompletion = async () => {
    ...
    try {
        const res = await api.post("api/ConfirmCompletion", {
            order_id: order_id.value,
        });
        if (res.status === 200) {
            alert("订单已完成确认");
