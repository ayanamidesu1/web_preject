<template>
    <div class="comic_page">
        <h1>插画浏览历史</h1>
        <div class="item_list">
            <div class="item" v-for="(item, index) in comic_item" :key="index">
                <div class="work_cover">
                    <img :src="'https://www.sunyuanling.com/server/static/image/comic/thumbnail/' + item.work.content_file_list.split(/[,，]/)[0]"
                        alt="封面">
                    <div class="page_count">
                        <span>{{ item.work.content_file_list.split(/[,，]/).length }}</span>
                        <img src="https://www.sunyuanling.com/assets/page_count.svg" alt="页数" class="icon"
                            style="width: 15px;">
                    </div>
                ...
                <div class="work_info">
                    <div class="work_name">
                        <span @click="open_comic_content_page(item.work.id)">{{ item.work.work_name }}</span>
                    </div>
                    <span>作品ID：{{ item.work.id }}</span>
                    <div class="work_tags">
