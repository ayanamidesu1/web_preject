...
async function get_like_status(item) {
    ...
    like_status.value = await user_interaction.like_work(work_id.value, 'add', token, 'comic', work_info.value.name) == 1 ? true : false
}
async function get_collect_status(item) {
    collect_status.value = item
    collect_status.value = await user_interaction.collect_work(
        work_id.value, 'add', token, 'comic', work_info.value.name) == 1 ? true : false;
}
...
onMounted(async () => {
    ...
    like_status.value = await user_interaction.like_work(
        work_id.value, 'search', token, 'comic', work_info.value.name) == 1 ? true : false;
    collect_status.value = await user_interaction.collect_work(
        work_id.value, 'search', token, 'comic', work_info.value.name) == 1 ? true : false;
    work_data.value.watch = await user_interaction.watch_work(work_id.value, token, 'comic', work_info.value.name)
    work_data.value.like = await user_interaction.like_work(work_id.value, 'count', token, 'comic', work_info.value.name)
    work_data.value.collect = await user_interaction.collect_work(work_id.value, 'count', token, 'comic', work_info.value.name)
});
watch(work_id, async () => {
    await get_work_info();
    await user_interaction.watch_work(parseInt(work_id.value,), token, 'comic', work_info.value.name);
