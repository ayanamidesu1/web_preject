    ...
    <div class="scroll_box_content">
      ...
      <div class="list" ref="list">
        <div class="item" v-for="(item, index) in props.msg_list" :key="index">
          <div v-if="props.msg_type === 'image'" class="image_item" @click="chose_item(item.work_info.id)">
            <div class="ranking">
              {{ index+1 }}
            </div>
            <img :src="'https://www.sunyuanling.com/server/static/image/comic/thumbnail/' + item.work_info.content_file_list.split(/[,，]/)[0]"
              class="image">
